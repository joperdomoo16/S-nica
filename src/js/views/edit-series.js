...
export function renderEditSeries(container, seriesId) {
  ...
  async function loadSeries() {
    try {
      ...
      if (data.creatorId !== user.uid) {
        alert("No tienes permiso para editar esta serie.");
        window.location.hash = '#/creator';
        return;
      }
      if (data.status !== 'draft' && data.status !== 'rejected' && data.status !== 'in_review') {
        alert("Esta serie ya está publicada y no se puede editar aquí.");
        window.location.hash = '#/creator';
        return;
      }
      ...
      category = data.category || 'Ciencia Ficción';
  ...
  function render() {
    if (isLoading) {
      container.innerHTML = `<div class="min-h-screen flex items-center justify-center bg-surface-dim"><div class="animate-spin text-primary">${icon('loader2', { size: 40 })}</div></div>`;
      return;
    }
      ...
      <div class="bg-surface-dim min-h-screen text-white pb-32">
        <div class="px-6 pt-8 max-w-lg mx-auto space-y-8">
          ...
          <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-black font-headline uppercase italic">Editar Serie</h1>
          </div>
  ...
  async function saveSeries(status) {
    ...
    try {
      ...
      await updateDoc(doc(db, 'series', seriesId), docData);
